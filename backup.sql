...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_asociacion`(
IN i_opcion integer,
IN i_rol integer,
IN i_menu integer,
IN i_usuario char(14),
IN i_empleado integer,
IN i_submenu integer
)
BEGIN
IF (i_opcion = 1) THEN
select mnu_nombre as MENU_PRINCIPAL, sub_nombre as SUB_MENU from frm_submenu sb, frm_menu m, frm_submenu_rol rm
where sb.mnu_id = m.mnu_id
and   sb.sub_id = rm.sub_id
and   rol_id = i_rol;
END IF;
IF (i_opcion = 2) THEN
select sub_id, sub_nombre from frm_submenu
where    mnu_id = i_menu;
END IF;
IF (i_opcion = 3) THEN
insert into frm_submenu_rol values(i_submenu, i_rol);
END IF;
IF (i_opcion = 4) THEN
select emp_codigo from frm_empleado where emp_usr = i_usuario into @w_emp_codigo;
if exists(select 1 from frm_usr_rol where rol_id = i_rol and emp_codigo =  @w_emp_codigo) then
update frm_usr_rol
set usr_estado = 'V'
where emp_codigo = (select emp_codigo from frm_empleado where emp_usr = i_usuario)
and rol_id= i_rol;
else
insert into frm_usr_rol values(i_rol,@w_emp_codigo, now(), 'V' );
end if;
END IF;
IF (i_opcion = 5) THEN
select ro.rol_id ROL, ro.rol_descripcion DESCRIPCION, i_usuario USUARIO, ur.usr_estado ESTADO
from frm_usr_rol ur, frm_rol ro
where ur.emp_codigo = (select emp_codigo from frm_empleado where emp_usr = i_usuario)
and ro.rol_id = ur.rol_id ;
END IF;
IF (i_opcion = 6) THEN
delete from frm_submenu_rol where sub_id = i_submenu and rol_id= i_rol;
END IF;
IF (i_opcion = 7) THEN
update frm_usr_rol
set usr_estado = 'E'
...
and rol_id= i_rol;
END IF;
 IF (i_opcion = 8) THEN  /*CATALOGO MENU*/
  SELECT mnu_id, mnu_nombre FROM frm_menu;
 END IF;

 IF (i_opcion = 9) THEN  /*CATALOGO ROL*/
   SELECT rol_id, rol_descripcion FROM frm_rol;
 END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_consulta_cv`(
in i_tipo char(1), /*identifica si es compra o venta*/
in i_opcion int, /*la opcion del SP*/
 in id_cv    int
)
BEGIN
...
IF (i_tipo  = 'V') THEN
  IF (i_opcion = 1) THEN
    select vta_codigo as Codigo,
    vta_fecha as Fecha,
    vta_total as Total,
    suc_nombre as Bodega,
    emp_nombre as Empleado
     from frm_venta v , frm_empleado e, frm_sucursal s
    where v.vta_codigo = id_cv
    and  v.emp_codigo = e.emp_codigo
    and  v.suc_codigo = s.suc_codigo  ;
  END IF;

  IF (i_opcion = 2 ) THEN

   select d.vta_codigo as Venta, p.pro_nombre as Producto,
   p.pro_descripcion AS Descripcion, pr.pre_descripcion AS Presentacion,
   d.vdet_cantidad as Cantidad, d.vdet_subtotal as SubTotal
   from frm_vdetalle d, frm_producto p, frm_presentacion pr
    where d.vta_codigo = id_cv
   and pr.pre_codigo = pro_presentacion
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_empleado`(
IN i_tipo integer,
IN i_codigo integer,
IN i_nombre varchar(120),
IN i_tel varchar(20),
IN i_dir varchar(120),
IN i_sueldo float,
IN i_puesto varchar(40),
IN i_usr char(14),
IN i_clave varchar(80),
IN i_estado char(1)
)
BEGIN
IF (i_tipo = 1) THEN
select ifnull(max(emp_codigo), 0) + 1 from frm_empleado into @w_emp;
insert into frm_empleado values(@w_emp, i_nombre, i_tel, i_dir, i_sueldo, i_puesto, i_usr, md5(i_clave), i_estado);
END IF;
IF (i_tipo = 2) THEN
if (i_clave = 'MISMA' ) THEN
update frm_empleado
set emp_nombre = i_nombre,
emp_tel = i_tel,
emp_direccion = i_dir,
emp_sueldo = i_sueldo,
emp_puesto = i_puesto,
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_finanzas`(
...
IN i_fecha_ini VARCHAR(10),

IN i_fecha_fin VARCHAR(10)
...
IF (i_tipo = 'I') THEN /*INGRESOS BANCO GENERAL*/
   ...
   IF (i_opcion  = 1) THEN /*CONSULTA DE -INGRESOS -BANCO -GENERAL*/



     SELECT ie_monto as MONTO, ie_descripcion AS DESCRIPCION, ie_fecha as FECHA



     FROM frm_ingresos_egresos
     ...
     AND date_format(ie_fecha, '%Y-%m-%d') >= date_format(i_fecha_ini, '%Y-%m-%d')

     AND date_format(ie_fecha, '%Y-%m-%d') <= date_format(i_fecha_fin, '%Y-%m-%d')
     ...
     SELECT vta_total, 'VENTA PRODUTO', vta_fecha



     FROM frm_venta

     WHERE date_format(vta_fecha, '%Y-%m-%d') >= date_format(i_fecha_ini, '%Y-%m-%d')

     AND date_format(vta_fecha, '%Y-%m-%d') <= date_format(i_fecha_fin, '%Y-%m-%d')
   ...
   IF (i_opcion = 2) THEN /*CONSULTA -INGRESOS -CONSOLIDADO  -GENERAL*/
    ...
    SELECT ifnull(count(*),0), ifnull(sum(ie_monto),0)
    ...
    INSERT INTO temporal VALUES ('CAPITAL', @w_monto, @w_numero);
    ...
    INSERT INTO temporal VALUES ('BANCO', @w_tot, @w_num);
    ...
    INSERT INTO temporal VALUES ('CAJA_CHICA', @w_monto, @w_numero);
    ...
    SELECT tm_cuenta as CUENTA, tm_cantidad as NUMERO, tm_monto as MONTO FROM temporal;
   ...
   IF (i_opcion = 3) THEN /*CONSULTA -INGRESOS -CONSOLIDADO -PARCIAL (NO COMPRA-VENTA)*/
    ...
    INSERT INTO temporal VALUES ('BANCO', @w_monto, @w_numero);
   ...
   IF (i_opcion = 4
   ...
   ) THEN /*CONSULTA -INGRESOS -CONSOLIDADO -SOLO COMPRA-VENTA*/
    ...
    SELECT ifnull(count(*), 0), ifnull(sum(vta_total), 0)
    ...
    INSERT INTO temporal VALUES ('VENTA PRODUCTO', @w_monto, @w_numero);
  ...
  /********************************************************************************/
    ...
    IF (i_opcion = 5) THEN /*CONSULTA -INGRESOS -BANCO -PARCIAL*/
 ...
 ;
   ...
   /********************************************************************************/
    ...
    IF (i_opcion = 6) THEN /*CONSULTA -INGRESOS -BANCO -COMPRA-VENTA */
 ...
 ;
   ...
   END IF;
    ...
    IF (i_opcion = 7) THEN /*CONSULTA -INGRESOS -CAJA CHICA */
 ...
 ;
   ...
   END IF;
    ...
    IF (i_opcion = 8) THEN /*CONSULTA -INGRESOS -CAPITAL */
...
IF (i_tipo = 'E') THEN
    ...
    IF (i_opcion = 1) THEN /*CONSULTA EGRESOS CONSOLIDADO GENERAL*/
      ...
      insert into temporal values('CAJA CHICA', @w_monto, @w_numero);
      ...
      SELECT ifnull(COUNT(*),0), ifnull(sum(com_total),0)



      FROM frm_compra

      WHERE date_format(com_fecha, '%Y-%m-%d') >= date_format(i_fecha_ini, '%Y-%m-%d')

      AND date_format(com_fecha, '%Y-%m-%d') <= date_format(i_fecha_fin, '%Y-%m-%d')
    ...
    /********************************************************************************/
     ...
     IF (i_opcion = 2) THEN /*CONSULTA EGRESOS CONSOLIDADO PARCIAL*/
     ...
     IF (i_opcion = 3) THEN /*CONSULTA EGRESOS CONSOLIDADO SOLO COMPRA-VENTA*/
      ...
      insert into temporal values ('COMPRA PRODUCTO', @w_monto, @w_numero);
     ...
     IF (i_opcion = 4) THEN /*CONSULTA EGRESOS BANCO SOLO COMPRA-VENTA*/
       ...
       SELECT com_total AS MONTO, 'COMPRA PRODUCTO' AS DESCRIPCION, com_fecha AS FECHA
 ...
 ;
     ...
     IF (i_opcion = 5) THEN /*CONSULTA EGRESOS BANCO SOLO PARCIAL*/
 ...
 ;
     ...
     IF (i_opcion = 6) THEN /*CONSULTA EGRESOS BANCO  GENERAL*/
         ...
         SELECT com_total, 'COMPRA PRODUCTO', com_fecha
 ...
 ;
    ...
    IF (i_opcion = 7) THEN /*CONSULTA -EGRESOS -CAJA CHICA */
 ...
 ;
   ...
   END IF;
    ...
    IF (i_opcion = 8) THEN /*CONSULTA -EGRESOS -CAPITAL */
...
IF (i_tipo = 'B') THEN
   ...
   IF (i_opcion = 1) THEN /*BALANCE CONSOLIDADO SOLO COMPRA-VENTA*/
    ...
    AND date_format(vta_fecha, '%Y-%m-%d') <= date_format(i_fecha_fin, '%Y-%m-%d')
     ...
     insert into temporal values (@w_monto1, @w_monto2, ifnull(@w_monto1 - @w_monto2, 0));
     ...
     SELECT tm_cuenta1 as 'VENTA PRODUCTO', tm_cuenta2 as 'COMPRA PRODUCTO', tm_cuenta3 as DIFERENCIA from temporal;
   ...
   IF (i_opcion = 2)  THEN /*BALANCE CONSOLIDADO PARCIAL */
    ...
    INSERT INTO temporal VALUES('INGRESOS', @w_monto2, @w_monto1, @w_monto3);
    ...
    INSERT INTO temporal VALUES('EGRESOS', @w_monto2, @w_monto1, @w_monto3);



    SELECT tm_caja_chica from temporal where tm_descripcion = 'INGRESOS' into @i_cchica;



    SELECT tm_banco from temporal where tm_descripcion = 'INGRESOS' into @i_banco;



    SELECT tm_capital from temporal where tm_descripcion = 'INGRESOS' into @i_capital;
    ...
    SELECT tm_caja_chica from temporal where tm_descripcion = 'EGRESOS' into @e_cchica;



    SELECT tm_banco from temporal where tm_descripcion = 'EGRESOS' into @e_banco;



    SELECT tm_capital from temporal where tm_descripcion = 'EGRESOS' into @e_capital;
    ...
    insert into temporal VALUES('DIFERENCIA', ifnull(@i_cchica - @e_cchica,0), ifnull(@i_banco - @e_banco, 0), ifnull(@i_capital- @e_capital, 0));




    SELECT tm_descripcion as TIPO,



           tm_caja_chica as CAJA_CHICA,
  ...
  IF (i_opcion = 3)  THEN /*BALANCE CONSOLIDADO GENERAL */
    ...
    INSERT INTO temporal VALUES('INGRESOS', @w_monto2, @w_tot, @w_monto3);
   ...
   SET @w_tot = @w_monto1 + @w_monto;
    ...
    INSERT INTO temporal VALUES('EGRESOS', @w_monto2, @w_tot, @w_monto3);
    ...
    SELECT tm_descripcion as DESCRIPCION, tm_caja_chica as CAJA_CHICA, tm_banco  as BANCO, tm_capital as CAPITAL
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_insertar_producto`(
IN i_codigo VARCHAR(20),
IN i_nombre varchar(60),
IN i_descripcion varchar(160),
IN i_existencia integer,
IN i_precio_venta float,
IN i_categoria integer,
IN i_presentacion integer,
IN i_precio_compra float,
in i_total  float,
IN i_parent varchar(20),
in i_proveedor integer,
IN i_tipo       integer,
in i_sucursal   integer
)
BEGIN
if (i_tipo = 1) then
start transaction;
if (i_parent= '') then
INSERT INTO frm_producto VALUES(i_codigo, i_nombre, i_descripcion, i_existencia, i_precio_venta, i_presentacion, i_categoria, i_codigo, 'V');
insert into frm_existencia values(i_codigo, i_sucursal, i_existencia);
else
INSERT INTO frm_producto VALUES(i_codigo, i_nombre, i_descripcion, i_existencia, i_precio_venta, i_presentacion, i_categoria, i_parent, 'V');
insert into frm_existencia values(i_codigo, i_sucursal, i_existencia);
end if;
if (i_existencia > 0) then
select ifnull(max(com_id),0) + 1 from frm_compra into @w_id_compra;
insert into frm_compra values(@w_id_compra, now(), i_existencia*i_precio_compra, 'A', i_proveedor);
insert into frm_cdetalle values(i_existencia,i_existencia*i_precio_compra, i_precio_compra, i_codigo, @w_id_compra );
end if;
COMMIT;
end if;
if (i_tipo = 4) then
select ifnull(max(com_id),0) + 1 from frm_compra into @w_id_compra;
insert into frm_compra values(@w_id_compra, now(), i_total, 'X', i_proveedor);
end if;
if (i_tipo = 5) then
select ifnull(max(com_id),0) from frm_compra into @w_id_compra;
insert into frm_cdetalle values(i_existencia,i_precio_venta, i_precio_compra, i_codigo, @w_id_compra );

 if exists (select 1 from frm_producto where pro_codigo = i_codigo and pro_estado = 'V') then
   UPDATE frm_existencia
   set exi_existencia = exi_existencia + i_existencia
...
update frm_compra
set com_estado = 'A'
where com_id = @w_id_compra;
end if;
if (i_tipo = 2) then
update frm_producto
set pro_nombre = i_nombre,
pro_descripcion = i_descripcion,
pro_existencia = i_existencia,
pro_precio_venta = i_precio_venta,
pro_presentacion = i_presentacion,
pro_categoria = i_categoria,
pro_parent = i_parent
where pro_codigo = i_codigo
and  pro_estado = 'V';
end if;
...
if (i_tipo = 11) then 
SELECT trs_cant_origen, trs_cant_destino
FROM frm_prm_traslado
WHERE pro_cod_origen = i_codigo
AND   pro_cod_destino = i_parent
into @w_cant_origen, @w_cant_destino;
SELECT exi_existencia
FROM frm_existencia
WHERE pro_codigo = i_codigo and suc_codigo = i_sucursal into @w_existencia;
IF (@w_existencia >= @w_cant_origen) then
start transaction;

/*UPDATE frm_producto
set pro_existencia = pro_existencia - @w_cant_origen
where pro_codigo = i_codigo
and   pro_estado = 'V';*/
update frm_existencia
set exi_existencia = exi_existencia - @w_cant_origen
where pro_codigo = i_codigo
and suc_codigo = i_sucursal;

/*update frm_producto
set pro_existencia = pro_existencia + @w_cant_destino
where pro_codigo = i_parent
and   pro_estado = 'V';*/

update frm_existencia
set exi_existencia = exi_existencia + @w_cant_destino
where pro_codigo = i_parent
and suc_codigo = i_sucursal;

select ifnull(max(tra_id),0)+1 from frm_traslado into @w_count;
insert into frm_traslado values(@w_count, now(), i_nombre, i_codigo, i_parent, i_sucursal);
commit;
END IF;
end if;
END$$
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_insertar_venta`(
in i_opcion int,
IN i_total float,
IN i_cantidad integer,
IN i_descuento float,
IN i_subtotal float,
IN i_cte_codigo varchar(25),
IN i_suc_codigo integer,
IN i_emp_codigo integer,
IN i_pro_codigo  varchar(20)
)
BEGIN
if (i_opcion = 1) then 
select ifnull(count(vta_codigo),0) + 1 from frm_venta
into @w_id_venta;
insert into frm_venta values(@w_id_venta, now(),  i_total,i_emp_codigo , i_suc_codigo , i_cte_codigo,'X');
end if;

if (i_opcion = 21) then
select ifnull(count(vta_codigo), 0) from frm_venta into @w_id_venta;
insert into frm_vdetalle values(i_cantidad, i_descuento, i_subtotal, i_pro_codigo, @w_id_venta);
update frm_producto
set pro_existencia = pro_existencia - i_cantidad
where pro_codigo = i_pro_codigo;
end if;
...
insert into frm_vdetalle values(i_cantidad, i_descuento, i_subtotal, i_pro_codigo, @w_id_venta);
update frm_existencia
set exi_existencia = exi_existencia - i_cantidad
where pro_codigo = i_pro_codigo
and   suc_codigo = i_suc_codigo;
end if;


if (i_opcion = 3) then
select ifnull(count(vta_codigo), 0) from frm_venta
into @w_i_venta;
update frm_venta
set  vta_estado = 'A'
where vta_codigo = @w_i_venta;
end if;
END$$
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_mant_producto`(
i_opcion int
)
BEGIN
if (i_opcion = 1) then
SELECT * FROM frm_presentacion;
end if ;
if (i_opcion = 2) then
SELECT * FROM frm_categoria;
end if;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_obtener_sucursal`(
IN i_opcion int)
BEGIN

if (i_opcion = 1) then
 select * from frm_sucursal;
end if;
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_obtiene_producto`(
i_tipo int,
i_nom  varchar(120),
i_sucursal int)
BEGIN
if (i_tipo = 1) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, pro_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
frm_presentacion.pre_descripcion AS PRESENTACION, frm_categoria.cat_descripcion as CATEGORIA, pro_parent as PADRE
from frm_producto, frm_presentacion, frm_categoria, frm_existencia
where frm_producto.pro_presentacion = frm_presentacion.pre_codigo
and  frm_producto.pro_codigo = frm_existencia.pro_codigo
and  frm_producto.pro_categoria = frm_categoria.cat_codigo
and  frm_producto.pro_estado = 'V'
and  frm_existencia.suc_codigo = i_sucursal;
end if;
if (i_tipo = 2) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, exi_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
frm_presentacion.pre_descripcion AS PRESENTACION, frm_categoria.cat_descripcion as CATEGORIA, pro_parent as PADRE
from frm_producto, frm_presentacion, frm_categoria, frm_existencia
where frm_producto.pro_presentacion = frm_presentacion.pre_codigo
and  frm_producto.pro_categoria = frm_categoria.cat_codigo
...
and  frm_producto.pro_estado = 'V'
and  frm_existencia.suc_codigo = i_sucursal
and  frm_categoria.cat_descripcion like concat('%', i_nom, '%');
end if;
if (i_tipo = 3) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, exi_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
...
and  frm_producto.pro_estado = 'V'
and  frm_existencia.suc_codigo = i_sucursal
and  frm_presentacion.pre_descripcion like concat('%', i_nom, '%');
end if;
if (i_tipo = 4) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, exi_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
...
and  frm_producto.pro_estado = 'V'
and  frm_existencia.suc_codigo = i_sucursal
and  frm_producto.pro_nombre like concat('%', i_nom, '%');
end if;
if (i_tipo = 44) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, exi_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
frm_presentacion.pre_descripcion AS PRESENTACION, frm_categoria.cat_descripcion as CATEGORIA
from frm_producto, frm_presentacion, frm_categoria, frm_existencia
where frm_producto.pro_presentacion = frm_presentacion.pre_codigo
and  frm_producto.pro_categoria = frm_categoria.cat_codigo
and  frm_producto.pro_estado = 'V'
...
and  frm_existencia.suc_codigo = i_sucursal
and  frm_producto.pro_estado = 'V'
and  frm_producto.pro_codigo like concat('%', i_nom, '%');
end if;
if (i_tipo = 55) then
select frm_producto.pro_codigo as CODIGO, pro_nombre as NOMBRE,
pro_descripcion as DESCRIPCION, exi_existencia as EXISTENCIA, pro_precio_venta as PRECIO_VENTA,
...
if (i_tipo = 8) then
select pro_nombre, pro_existencia, pro_precio_venta,pre_descripcion, pro_descripcion
from frm_producto p, frm_presentacion pr
where pro_codigo = i_nom
and  p.pro_presentacion = pr.pre_codigo
and  p.pro_estado = 'V';
end if;

if (i_tipo = 81) then
select pro_nombre, exi_existencia, pro_precio_venta,pre_descripcion, pro_descripcion
from frm_producto p, frm_presentacion pr, frm_existencia e
where p.pro_codigo = i_nom
and p.pro_codigo = e.pro_codigo
and e.suc_codigo = i_sucursal
and  p.pro_presentacion = pr.pre_codigo
and  p.pro_estado = 'V';
end if;
...
where frm_producto.pro_presentacion = frm_presentacion.pre_codigo
and  frm_producto.pro_categoria = frm_categoria.cat_codigo
and  frm_producto.pro_codigo = i_nom
and  frm_producto.pro_codigo = frm_existencia.pro_codigo
and  frm_existencia.suc_codigo = i_sucursal
and  frm_producto.pro_estado = 'V';
end if;
if (i_tipo = 10) then
select pro_cod_destino, pro_cod_destino
from frm_prm_traslado
where pro_cod_origen = i_nom;
end if;
END$$
...
CREATE DEFINER=`root`@`%` PROCEDURE `sp_sucursal`(
in i_opcion int,
in i_nombre varchar(60),
in i_direccion varchar(180),
in i_zona  int,
in i_dep   int
)
BEGIN
if (i_opcion = 1) then
select ifnull(max(suc_codigo), 0) + 1 from frm_sucursal into @w_cont;
insert into frm_sucursal values(@w_cont, i_nombre, i_direccion, i_zona, i_dep);
end if;
if (i_opcion = 2) then
delete from frm_sucursal
where suc_codigo = i_zona;
end if;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_traslado_producto`(
in i_opcion  int,
in i_sucursal_envia int,
in i_sucursal_recibe  int,
in i_cantidad  int,
in i_pro_codigo varchar(20),
in i_usr char(14)
)
BEGIN
 ...
 if (i_opcion = 1) then
  start transaction;
   ...
   set exi_existencia = exi_existencia - i_cantidad
   where pro_codigo = i_pro_codigo
   and suc_codigo = i_sucursal_envia;

   update frm_existencia
   set exi_existencia = exi_existencia + i_cantidad
   where pro_codigo = i_pro_codigo
   and suc_codigo = i_sucursal_recibe;

   insert into frm_traslado_producto values (0, i_usr, now(), i_pro_codigo, i_sucursal_envia, i_sucursal_recibe, i_cantidad);
...
CREATE DEFINER=`root`@`%` FUNCTION `fnc_cambio_clave`(
i_opcion int,
i_clave_ant varchar(80),
i_clave_nueva varchar(80),
i_cod_emp int
) RETURNS varchar(30) CHARSET latin1
BEGIN
IF (i_opcion = 1) then
IF EXISTS (SELECT 1 FROM frm_empleado WHERE  emp_codigo = i_cod_emp AND emp_clave = md5(i_clave_ant)) THEN
UPDATE frm_empleado
SET emp_clave = md5(i_clave_nueva)
WHERE emp_codigo = i_cod_emp;
RETURN 'CONTRASEÑA CAMBIADA CON EXITO';
ELSE
RETURN 'CONTRASEÑA ACTUAL INVALIDA';
END IF;
END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fnc_insertar_producto`(
 i_codigo VARCHAR(20),
 i_nombre varchar(60),
 i_descripcion varchar(160),
 i_existencia integer,
 i_precio_venta float,
 i_categoria integer,
 i_presentacion integer,
 i_precio_compra float,
 i_total  float,
 i_parent varchar(20),
 i_proveedor integer,
 i_tipo       integer,
...
CREATE DEFINER=`root`@`localhost` FUNCTION `fnc_insertar_venta`(
 i_opcion int,
 i_total float,
 i_cantidad integer,
 i_descuento float,
 i_subtotal float,
 i_cte_codigo varchar(25),
 i_suc_codigo integer,
 i_emp_codigo integer,
 i_pro_codigo  varchar(20)

) RETURNS varchar(30) CHARSET latin1
BEGIN

/*if (i_opcion = 1) then
select ifnull(max(vta_codigo),0) + 1 from frm_venta
into @w_id_venta;
insert into frm_venta values(@w_id_venta, now(),  i_total,i_emp_codigo , i_suc_codigo , i_cte_codigo,'X');
set @w_id_venta_cast = convert(varchar(30),@w_id_venta);
end if;
*/

/*
if (i_opcion = 21) then
select ifnull(max(vta_codigo), 0) from frm_venta into @w_id_venta;
insert into frm_vdetalle values(i_cantidad, i_descuento, i_subtotal, i_pro_codigo, @w_id_venta);
update frm_producto
set pro_existencia = pro_existencia - i_cantidad
where pro_codigo = i_pro_codigo;
...
if (i_opcion = 3) then
select ifnull(max(vta_codigo), 0) from frm_venta
into @w_i_venta;
update frm_venta
set  vta_estado = 'A'
where vta_codigo = @w_i_venta;
